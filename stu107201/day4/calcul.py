...
def mul_div(arg):
    ...
    md = re.search('\d+[\*\/]\d+',arg[0])
    #没匹配到就直接返回
    if not md:
        return arg
    #将匹配到的内容保存在content中
    content = re.search('\d+[\*\/]\d+',arg[0]).group()
    #对匹配到的内容进行计算
    if len(content.split('*'))>1:
        n1,n2 = content.split('*')
        computere = int(n1) * int(n2)
    else:
        n1,n2 = content.split('/')
        computere = int(int(n1) / int(n2))

    #取出匹配内容两头的内容：before，after
    before,after = re.split('\d+[\*\/]\d+',arg[0],1)
    #然后拼接成新的字符串
...
#对字符串进行匹配，匹配从左至右最近的两个数字的加减
    ad = re.search('\d+[\+\-]{1}\d+',arg[0])
        #如果没匹配到就直接发回
    if not ad:
        return arg
    content = re.search('\d+[\+\-]{1}\d+',arg[0]).group()
    #对匹配的内容进行加减计算
    if len(content.split('+'))>1:
         n1,n2 =content.split('+')
         computere = int(n1)+int(n2)
    ...
    before,after = re.split('\d+[\+\-]{1}\d+',str(arg[0]),1)
